...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    # key = name 
    # value = a list [num_genes(0/1/2), trait(T/F)]
...
def normalize(probabilities):
    ...
    """
    for _, data in probabilities.items():
        gene_factor = float(1 / sum(data["gene"].values()))
        trait_factor = float(1 / sum(data["trait"].values()))

        data["gene"] = {k: (v * gene_factor) for k, v in data["gene"].items()}
        data["trait"] = {k: (v * trait_factor) for k, v in data["trait"].items()} 
